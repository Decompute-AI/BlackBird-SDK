...
# Local imports
from mlx_parallm.mlx_parallm.sample_utils import top_p_sampling
from mlx_parallm.mlx_parallm.models.base import BatchedKVCache
...
def _get_classes(config: dict):
    ...
    try:
        arch = importlib.import_module(f"mlx_parallm.mlx_parallm.models.{model_type}")
    except ImportError:
...
def generate_step(
    ...
    if not isinstance(temp, (int, float)):
        raise TypeError(f"temp must be a number, got {type(temp)}")
    temp = float(temp)  # Convert to float to ensure proper division
    ...
    def sample(logits: mx.array) -> Tuple[mx.array, float]:
        ...
        if temp == 0:
            ...
            if top_p > 0 and top_p < 1.0:
                ...
                # Ensure temperature is a float for the division
...
def stream_generate(
    ...
    prompt: str,
    max_tokens: int = 100,
    temp: float = 0.0,  # Added explicit temperature parameter
    top_p: float = 1.0,  # Added explicit top_p parameter
    repetition_penalty: Optional[float] = None,
    repetition_context_size: Optional[int] = 20,
    logit_bias: Optional[Dict[int, float]] = None,
    **kwargs,  # Keep kwargs for any other parameters
    ...
    Args:
        ...
        repetition_penalty: Penalty for token repetition (default: None)
        repetition_context_size: Context size for repetition penalty (default: 20)
        logit_bias: Dictionary of token biases (default: None)
    """
    if not isinstance(tokenizer, TokenizerWrapper):
        tokenizer = TokenizerWrapper(tokenizer)

    prompt_tokens = mx.array(tokenizer.encode(prompt))
    # Ensure prompt_tokens is 2D
    if prompt_tokens.ndim == 1:
        prompt_tokens = prompt_tokens[None, :]

    detokenizer = tokenizer.detokenizer

    # Pass the parameters explicitly to generate_step
    generation_params = {
        'temp': temp,
        'top_p': top_p,
    ...
    generation_params.update(kwargs)  # Add any additional kwargs

    detokenizer.reset()
    for (token, prob), n in zip(
        generate_step(prompt_tokens, model, **generation_params),
        range(max_tokens),
    ):
